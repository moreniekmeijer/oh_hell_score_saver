...
class Game:
    ...
    def save_round_scores(self):
        if not self._history:
            snapshot = {p.name: (p.score, p.score) for p in self._players}
        else:
            prev_snapshot = self._history[-1]
            snapshot = {
                p.name: (p.score, p.score - prev_snapshot[p.name][0]) for p in self._players}
    ...
    def show_leaderboard(self):
        ...
        header = ["Round"] + [p.name for p in self._players]
        print(" | ".join(f"{h:>12}" for h in header))
        print("-" * (len(header) * 14))

        for i, snapshot in enumerate(self._history, start=1):
            row = [f"{i}"]
            for p in self._players:
                total, delta = snapshot[p.name]
                if delta >= 0:
                    cell = f"(+{delta}) {total}"
                else:
                    cell = f"({delta}) {total}"
...
def init_game():
    while True:
        try:
            raw_names = input(
                "Please fill in the names of the players (separated by spaces): ")
...
def play_round(game, round_number):
    ...
    ordered_players = game.players[starting_index:] + \
        game.players[:starting_index]

    while True:
        try:
            tricks = int(input(
                "With how many tricks do you wanna play? (Type 0 if you wanna skip this round) "))
            if tricks == 0:
            ...
            try:
                ...
                if i == (num_players - 1):
                    if (sum(bids.values()) + bid) == tricks:
                        print(
                            f"Sorry {player.name}, you cannot make the total equal to {tricks}. Choose another number.")
                        continue
    ...
    wait_for_enter(
        "Hit Enter when the round is played and you're ready to fill in the trick wins...\n")
    ...
    for i, player in enumerate(ordered_players):
        while True:
            try:
                wins = int(
                    input(f"{player.name}, how many tricks did you win? "))
                if wins < 0:
                ...
                if wins > tricks:
                    print(
                        "Don't lie! Cannot win more than the amount of tricks this round")
                    continue

                if i == (num_players - 1):
                    if sum(total_wins.values()) + wins != tricks:
                        print(
                            f"Sorry {player.name}, the total wins must equal the total tricks ({tricks}).")
                        redo = input(
                            "Type 'r' to redo the whole round, or Enter to try again: ").strip().lower()
                        if redo == "r":
